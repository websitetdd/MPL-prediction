...
const PageAdmin = (() => {
  const SCORE_OPTS = ["2-0", "2-1", "1-2", "0-2"];
  ...
  function renderMatchLists() {
    ...
    sched.innerHTML = open.length
      ? open.map((m) =>
          `<div class="leader-row">` +
          `<span class="lr-name"><b>${esc(matchTeamName(m.teamA))}</b> vs <b>${esc(matchTeamName(m.teamB))}</b>` +
    ...
    fin.innerHTML = done.length
      ? done.map((m) => {
          ...
          return (
            ...
            SCORE_OPTS.map((s) => `<option value="${s}">${s}</option>`).join("") +
            `</select>` +
            `<button class="btn btn-gold btn-sm" data-rapply="${esc(m.id)}">Apply result</button>` +
  ...
  function init() {
    ...
    $("#adminLoginForm").addEventListener("submit", (e) => {
      e.preventDefault();
      const res = Auth.adminLogin($("#adminUser").value.trim(), $("#adminPass").value);
